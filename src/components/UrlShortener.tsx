  ...
  return (
    <div className="w-full max-w-2xl mx-auto px-2">
      <div className="glass p-2 rounded-full shadow-2xl opacity-0 animate-fade-in-up delay-100">
        <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row w-full relative">
          <input
            type="url"
            ...
            placeholder={session ? "Paste your long link here..." : "Sign in to shorten URLs..."}
            value={url}
      ...
      {error && <p className="mt-6 text-center font-medium text-red-600 dark:text-red-500">{error}</p>}
      {existsMessage && <p className="mt-6 text-center font-medium text-emerald-600 dark:text-emerald-500">{existsMessage}</p>}
      ...
      {shortUrl && (
        <div className="mt-8 p-6 glass flex flex-col sm:flex-row items-center justify-between gap-4 max-w-2xl mx-auto opacity-0 animate-fade-in">
          <a href={shortUrl} target="_blank" rel="noopener noreferrer" className="break-all text-lg font-medium text-blue-600 transition-colors hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300">
            {shortUrl}
          </a>
          <button 
